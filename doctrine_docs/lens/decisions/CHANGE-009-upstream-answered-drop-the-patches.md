...
2026-08-02 に一周を終えた。見積との差は無い。

| 区分 | 見積 | 実測 |
|---|---|---|
| 消えた実装 | `titles.ts` 137 行 | 137 行（差し引き **−114 行**） |
| 書き直した文書 | 6 | 6 |
| 新しく作った文書 | 2（ADR-020・IMPACT-009） | 2 |

### 門の実測

| 門 | 結果 |
|---|---|
| 単体 | 206 件すべて通る（213 → 206。**継ぎの受入 7 件が消えたため**） |
| 突然変異 | 表の 50 件すべてを試験が捕まえる（53 → 50。`titles.ts` の 3 行が消えた） |
| 写し（画面） | 誤り 0・食い違い 0 |
| 統合 | 10 件すべて通る |
| 監査 | error 0・warn 0・advisory 1（環境の hooks） |
| 梱包 | 12 ファイル・61.35 KB |

**試験の数が減ったことを「退化」と読まないこと。** 消えたのは
「継ぎが正しく動くか」を見ていた受入である。継ぎが無くなれば、その受入も要らない。
守るべき性質（題名が id へ落ちる・節点が題名を持つ）は残してある。

### 順序が効いたこと

**プラグインを先に更新した。** 手元が 0.7.0 のまま継ぎを捨てていたら、
題名が全部 id へ落ちていた。更新前に実測して 0.7.0 のままだと確かめてから動いた。

**`ADR-018` が捨てる条件を書いていた。** 「届いたらこの層は捨てる」。
そのおかげで「まだ残しておこうか」の議論が起きなかった。
`ADR-020` はこれを規律にした——**捨てる条件を書かない継ぎは置かない。**
