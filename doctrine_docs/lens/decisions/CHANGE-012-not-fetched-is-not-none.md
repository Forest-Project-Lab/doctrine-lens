...
`CHANGE-010` の走査が挙げ、反証を三視点で当てても落ちなかった指摘のうち、
**六箇所と同じ系統の三つ**をこの一周で閉じた。

**一。`status` を言っていない節点を「非現行」と数えていた。**

上流は型も status も書かれていない文書に `""` を返す（既定を引けないため）。
`""` を「現行でない」と数えると、要約は「非現行 N」と言うのに
行には空文字が渡って何も出ない。**`ADR-021` 決定 2 の支えが、そこで崩れる。**

```
行の status="" / notCurrent=true / 要約の非現行=1
→ 数は 1 と言うが、画面に出る status は 0 本
```

上流が言っていないものは、判じられない（`null`）。`ADR-023` 決定 1 と同じ形である。

**二。起点が循環に落ちると、畳んだ件数の総和が 1 足りなかった。**

`unreached` の式が起点を二重に引いていた（起点が `inCycle` に数えられているのに、
さらに `-1` する）。`SPEC-006` 制約「畳んだら必ず件数を書く」が、そこで破れる。

```
直す前: 節点 3 = 行 0 + 循環 2 + 前提 0 + 届かない 0 = 2
直した後: 節点 3 = 行 0 + 循環 2 + 前提 0 + 届かない 1 = 3
```

**三。文書三つが揃って、実在しない実測を挙げていた。**

`SPEC-006`・`ADR-014`・`CHANGE-005` が同文で
「`DECIDED-001` を起点に置くと 17 行」と書いていたが、**`DECIDED-001` は辺を
一本も持たず、実測は 0 行**である。正しい起点は `REQ-002`（19 行・非現行 4 件・
波 3・4・4・5 まで一致）だった。

**三つの文書が同じ誤りを持ち、誰も測り直さなかった。**
`tools/check-measured-claims.mjs` を作って門にした——
文書が挙げる「`<id>` を起点に置くと `<N>` 行」を、毎回組み立てて突き合わせる。
