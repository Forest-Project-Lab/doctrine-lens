...
実測で、これは誤読を生む。`DECIDED-001` を起点に置くと 17 行が出るが、
そのうち 4 件が非現行（`SPEC-002` が波 3、`SPEC-003` が波 4、`TEST-002` が波 4、
`TEST-003` が波 5）で、**現行と交互に混ざる。** 撤回して退役した文書が
「直すことになるもの」の一覧に並ぶ。
...
doctrine 側へ #152・#153・#154 を出した。これらは上流に概念や検査が無いので、
...
この変更のあいだに doctrine 側の issue 六本を、実測で裏づけて書き直した。
