...
```
SPEC-004 の帰結: IMPL-001, SPEC-005, SPEC-006, TEST-004, TEST-005, TEST-006
SPEC-006 の帰結: IMPL-001, TEST-006
```
...
| `SPEC-005`・`TEST-005` | **帰結だが直さない。** `traceFindings` の呼び方が変わらないため。理由をここに残す | 現行のまま |
